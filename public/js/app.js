    ...
    script.onload = () => {
      ...
      const intervalId = setInterval(() => {
        ...
        if (window.jspdf && typeof window.jspdf.jsPDF === 'function') {
          ...
          console.error(`jsPDF library not found within timeout while loading from ${scriptSrc}.`);
          clearInterval(intervalId);
          reject(new Error(`jsPDF library not found within timeout while loading from ${scriptSrc}`));
        }
      }, pollInterval);
    };
...
function setupRealTimeOrderFilters() {
    ...
    if (orderStatusFilter) {
        ...
        console.error('Order status filter element not found');
    }
    ...
    if (orderSupplierFilter) {
        ...
        console.error('Order supplier filter element not found');
    }
}
