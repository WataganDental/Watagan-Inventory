...
function addQuickStockManualEntry() { 
    console.log('addQuickStockManualEntry called');
    
    try {
        const productNameInput = document.getElementById('quickStockProductName');
        const quantityInput = document.getElementById('quickStockQuantity');
        const batchList = document.getElementById('quickStockBatchList');
        
        if (!productNameInput || !quantityInput || !batchList) {
            console.warn('Required elements for manual entry not found');
            return;
        }
        ...
        if (!productName) {
            if (typeof uiEnhancementManager !== 'undefined') {
                uiEnhancementManager.showToast('Please enter a product name', 'error');
            }
            return;
        }
        ...
        if (isNaN(quantity) || quantity <= 0) {
            if (typeof uiEnhancementManager !== 'undefined') {
                uiEnhancementManager.showToast('Please enter a valid quantity', 'error');
            }
            return;
        }
        
        // Find matching product in inventory
        const matchingProduct = inventory.find(item => 
            item.name && item.name.toLowerCase().includes(productName.toLowerCase())
        );
        
        if (!matchingProduct) {
            if (typeof uiEnhancementManager !== 'undefined') {
                uiEnhancementManager.showToast('Product not found in inventory', 'warning');
            }
            return;
        }
        ...
        listItem.className = 'flex justify-between items-center p-2 bg-gray-100 dark:bg-gray-700 rounded mb-2';
        listItem.innerHTML = `
            <span>${matchingProduct.name} (Current: ${matchingProduct.quantity})</span>
            <span class="font-semibold">+${quantity}</span>
            <button onclick="this.parentElement.remove()" class="btn btn-error btn-xs">Remove</button>
        `;
        listItem.dataset.productId = matchingProduct.id;
        ...
        if (typeof uiEnhancementManager !== 'undefined') {
            uiEnhancementManager.showToast(`Added ${matchingProduct.name} (+${quantity}) to batch`, 'success');
        }
        
        console.log(`Added manual entry: ${matchingProduct.name} (+${quantity})`);
    } catch (error) {
        console.error('Error in addQuickStockManualEntry:', error);
    }
}
function submitQuickStockBatch() { 
    console.log('submitQuickStockBatch called');
    
    try {
        const batchList = document.getElementById('quickStockBatchList');
        if (!batchList) {
            console.warn('Batch list element not found');
            return;
        }
        
        const batchItems = batchList.querySelectorAll('[data-product-id]');
        if (batchItems.length === 0) {
            if (typeof uiEnhancementManager !== 'undefined') {
                uiEnhancementManager.showToast('No items in batch to submit', 'warning');
            }
            return;
        }
        ...
        if (updates.length === 0) {
            if (typeof uiEnhancementManager !== 'undefined') {
                uiEnhancementManager.showToast('No valid items to submit', 'error');
            }
            return;
        }
        ...
        Promise.all(updates.map(async update => {
            const product = inventory.find(item => item.id === update.productId);
            if (product) {
                const newQuantity = (product.quantity || 0) + update.quantity;
                
                await db.collection('inventory').doc(update.productId).update({
            ...
            // Update displays
            if (typeof inventoryManager !== 'undefined' && inventoryManager.loadInventory) {
                inventoryManager.loadInventory();
            }
            
            if (typeof uiEnhancementManager !== 'undefined') {
                uiEnhancementManager.showToast(`Successfully updated ${updates.length} products`, 'success');
            }
            
            console.log(`Submitted batch update for ${updates.length} products`);
        }).catch(error => {
            console.error('Error submitting batch updates:', error);
            if (typeof uiEnhancementManager !== 'undefined') {
                uiEnhancementManager.showToast('Error submitting batch updates', 'error');
            }
        });
        
    } catch (error) {
        console.error('Error in submitQuickStockBatch:', error);
    }
}
function handleQuickStockProductSearch() { 
    console.log('handleQuickStockProductSearch called');
    
    try {
        const searchInput = document.getElementById('quickStockProductSearch');
        const searchResults = document.getElementById('quickStockSearchResults');
        ...
        if (!inventory || inventory.length === 0) {
            searchResults.innerHTML = '<div class="p-2 text-gray-500">No inventory data available</div>';
        ...
        if (matchingProducts.length === 0) {
            ...
            searchResults.innerHTML = matchingProducts.map(product => `
                <div class="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer border-b" 
                     onclick="selectQuickStockProduct('${product.id}', '${product.name}', ${product.quantity})">
                    <div class="font-medium">${product.name}</div>
                    <div class="text-sm text-gray-500">Current: ${product.quantity} | ${product.supplier || 'Unknown Supplier'}</div>
                </div>
            `).join('');
        }
        ...
        console.error('Error in handleQuickStockProductSearch:', error);
    }
}

// Helper function for quick stock product selection
function selectQuickStockProduct(productId, productName, currentQuantity) {
    console.log(`selectQuickStockProduct called for ${productName}`);
    ...
    try {
        ...
        console.error('Error in selectQuickStockProduct:', error);
    }
}
